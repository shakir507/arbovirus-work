...
std::cout<<GLOBAL.NUM_DAYS<<std::endl;
...
// }
for(count_type time_step=GLOBAL.START_DAY;time_step<=GLOBAL.NUM_DAYS;time_step++)
     {
     ...
     rk4(t, y0, yf0, dy,dyf, h, y1,yf1, dif);
      ...
      if(int(time_step*h)==time_step*h)
      {
            for (int c = 1; c <= GLOBAL.Nodes; ++c){
                  ...
                  plot_data.nums["csvContent"].push_back({time_step, {c*1.0,y0[5*GLOBAL.Nodes+c]}});
            }
      }
     for(int i=1;i<=N;i++)
      {
